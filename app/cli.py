...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert a Google Doc to a Quarto Markdown (.qmd) ZIP"
    )
    ...
    parser.add_argument("--mode", default="paper", choices=["paper", "blog"],
                        help="Document mode: paper (with PDF) or blog (QMD only)")
    ...
    parser.add_argument(
        "--slug",
        default="",
        help="Output filename stem for blog mode (no extension, no spaces)",
    )
    parser.add_argument(
        "--render-pdf",
        action="store_true",
        dest="render_pdf",
        help="Run Quarto to render the PDF (paper mode only)",
    )
    parser.add_argument(
        "--output",
        default="output.zip",
    ...
    # ── Convert ───────────────────────────────────────────────────────────────
    print(f"[2/3] Converting to QMD ({args.mode} mode)...")
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if args.mode == "blog":
            ...
            qmd_content = convert_blog(doc, meta, images_dir, docx_bytes=docx_bytes)
            print("[3/3] Packaging (blog — QMD + images)...")
            zip_bytes = zip_blog(qmd_content=qmd_content, images_dir=images_dir, slug=slug)
        else:
            meta = {
                "title": args.title,
